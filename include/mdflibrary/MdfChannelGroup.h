...
class MdfChannelGroup {
 ...
 public:
  MdfChannelGroup(const mdf::IChannelGroup* group) { this->group = group; }
  ~MdfChannelGroup() { this->group = nullptr; }
  const mdf::IChannelGroup* GetGroup() { return group; }
  ...
  std::vector<MdfChannel> GetChannels() {
    ...
    std::vector<MdfChannel> channels(count);
    for (size_t i = 0; i < count; i++) channels[i] = MdfChannel(pChannels[i]);
    return channels;
  }
