...
TEST(TestHdComment, AlternativeName) {
  HdComment comment;

  constexpr std::string_view kTx = "Alternative Test";
  comment.Comment(MdString(kTx));
  EXPECT_EQ(comment.Comment().Text(), kTx);

  MdAlternativeName& names = comment.Names();
  EXPECT_FALSE(names.IsActive());

  constexpr std::string_view kName = "Birdy";
  {
    names.AddAlternativeName(MdString(kName));
    EXPECT_EQ(names.AlternativeNames().size(), 1);
  }

  constexpr std::string_view kDisplayName = "Road Runner";
  {
    names.AddDisplayName(MdString(kDisplayName));
    EXPECT_EQ(names.DisplayNames().size(), 1);
  }

  constexpr std::string_view kVendor = "ACME Corporation";
  {
    names.AddVendor(MdString(kVendor));
    EXPECT_EQ(names.Vendors().size(), 1);
  }

  constexpr std::string_view kDesc = "Coyote favorite meal";
  {
    names.AddDescription(MdString(kDesc));
    EXPECT_EQ(names.Descriptions().size(), 1);
  }
  EXPECT_TRUE(names.IsActive());
  const std::string xml_snippet = comment.ToXml();
  std::cout << xml_snippet << std::endl;
  ...
  const MdAlternativeName& names1 = comment1.Names();

  ASSERT_EQ(names1.AlternativeNames().size(), 1);
  EXPECT_EQ(names1.AlternativeNames()[0], kName);

  ASSERT_EQ(names1.DisplayNames().size(), 1);
  EXPECT_EQ(names1.DisplayNames()[0], kDisplayName);

  ASSERT_EQ(names1.Vendors().size(), 1);
  EXPECT_EQ(names1.Vendors()[0], kVendor);

  ASSERT_EQ(names1.Descriptions().size(), 1);
  EXPECT_EQ(names1.Descriptions()[0], kDesc);
}

TEST(TestHdComment, Formula) {
  HdComment comment;

  constexpr std::string_view kTx = "Formula Test";
  comment.Comment(MdString(kTx));
  EXPECT_EQ(comment.Comment().Text(), kTx);

  auto& formula = comment.Formula();
  EXPECT_FALSE(formula.IsActive());

  constexpr std::string_view kSyntax = "Birdy(x)";
  constexpr std::string_view kVersion = "1.23";
  constexpr std::string_view kSource = "ACME Corporation";

  constexpr std::string_view kVariable = "Coyote";

  constexpr std::string_view kCn = "CN Block";
  constexpr std::string_view kCs = "CN/SI Source";
  constexpr std::string_view kCp = "CN/SI Source Path";

  constexpr std::string_view kGn = "CG Block";
  constexpr std::string_view kGs = "CG/SI Source";
  constexpr std::string_view kGp = "CG/SI Source Path";

  constexpr std::array<uint64_t,3> kIndexList = {1,2,3};
  constexpr bool kRaw = true;
  {
    MdSyntax syntax;
    syntax.Syntax( std::string(kSyntax) );
    syntax.Version(std::string(kVersion) );
    syntax.Source(std::string(kSource) );
    formula.Syntax( syntax );
    EXPECT_EQ(syntax.Syntax(), kSyntax);
    EXPECT_EQ(syntax.Version(), kVersion);
    EXPECT_EQ(syntax.Source(), kSource);

    formula.AddCustomSyntax(syntax);
    EXPECT_EQ(formula.CustomSyntaxes().size(),1);

    MdVariable var;
    var.Variable( std::string(kVariable) );
    var.CnBlockName(std::string(kCn));
    var.CnSourceName(std::string(kCs));
    var.CnSourcePath(std::string(kCp));
    var.CgBlockName(std::string(kGn));
    var.CgSourceName(std::string(kGs));
    var.CgSourcePath(std::string(kGp));
    for (uint64_t index : kIndexList) {
      var.AddArrayIndex(index);
    }
    var.Raw(kRaw);
  ...
  const MdFormula& formula1 = comment1.Formula();
  const MdSyntax& syntax1 = formula1.Syntax();
  EXPECT_EQ(syntax1.Syntax(), kSyntax);
  EXPECT_EQ(syntax1.Version(), kVersion);

  EXPECT_EQ(formula.CustomSyntaxes().size(), 1);
  for (const auto& [name, syntax2] : formula.CustomSyntaxes()) {
    EXPECT_EQ(syntax2.Syntax(), kSyntax);
    EXPECT_EQ(syntax2.Version(), kVersion);
    EXPECT_EQ(syntax2.Source(), kSource);
  }

  EXPECT_EQ(formula.Variables().size(), 1);
  for (const auto& [name, var1] : formula.Variables()) {
    EXPECT_EQ(var1.Variable(), kVariable);
    EXPECT_EQ(var1.CnBlockName(), kCn);
    EXPECT_EQ(var1.CnSourceName(), kCs);
    EXPECT_EQ(var1.CnSourcePath(), kCp);
    EXPECT_EQ(var1.CgBlockName(), kGn);
    EXPECT_EQ(var1.CgSourceName(), kGs);
    EXPECT_EQ(var1.CgSourcePath(), kGp);
    const auto& index_list = var1.ArrayIndexes();
    ASSERT_EQ(index_list.size(), kIndexList.size());
    for (size_t index = 0; index < kIndexList.size(); ++index) {
      EXPECT_EQ(kIndexList[index], index_list[index] );
    }
    EXPECT_EQ(var1.Raw(), kRaw);
  }
}
} // end namespace