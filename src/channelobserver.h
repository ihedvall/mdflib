...
template <class T> class ChannelObserver : public IChannelObserver {
private:
  ...
  template <typename V> bool GetVirtualSample(size_t sample, V &value) const {
...
public:
  ChannelObserver(const IDataGroup &data_group, const IChannelGroup &group,
      ...
      : IChannelObserver(channel), data_group_(data_group),
        record_id_(group.RecordId()), value_list_(group.NofSamples(), T{}),
