...
protected:
  ...
  size_t ReadLinks3(std::FILE *file,
  ...
  void
  CreateMd4Block(); ///< Helper function that creates an MD4 block to this block

  template <typename T> std::size_t ReadNumber(std::FILE *file, T &dest) const {
    if (IsBigEndian()) {
      BigBuffer<T> buff;
