...
public:
  ...
  FileNameWithoutPath() const; ///< File name without path.
   ...
   */
  [[nodiscard]] std::string
  RootName() const { ///< Return the name of the root tag.
   ...
   */
  [[nodiscard]] const IXmlNode *
  GetNode(const std::string &tag) const; ///< Returns a tag node.

  virtual bool ParseFile(); ///< Parses the XML file.
  virtual bool
  ParseString(const std::string &input); ///< Parses a string instead of a file.
  virtual void Reset(); ///< Reset the internals i.e. ready for a new parsing.
  virtual bool WriteFile(); ///< Writes the XML file.
  std::string WriteString(
      bool skip_header = false); ///< Creates a string that is a whole XML file.
protected:
  ...
  virtual std::unique_ptr<IXmlNode>
  CreateNode(const std::string &name); ///< Creates a new node.
private:
 ...
 */
[[nodiscard]] std::unique_ptr<IXmlFile>
CreateXmlFile(const std::string &type = "Expat"); ///< Creates a XML parser
} // namespace mdf
