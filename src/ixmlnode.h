...
namespace {
template <typename T> T XValue(const std::string &value) {
  std::string v(value);
...
template <> bool XValue(const std::string &value) {
  std::string v(value);
  mdf::MdfHelper::Trim(v);
...
template <> std::string XValue(const std::string &value) {
  std::string v(value);
  mdf::MdfHelper::Trim(v);
  return v;
}
...
public:
  ...
  virtual void
  GetChildList(ChildList &child_list) const; ///< Returns the child nodes.
  [[nodiscard]] virtual const IXmlNode *
  GetNode(const std::string &tag) const; ///< Returns a node if it exist.
  ...
  virtual IXmlNode &
  AddUniqueNode(const std::string &name); ///< Adds a unique tag
...
protected:
  ...
  [[nodiscard]] virtual std::unique_ptr<IXmlNode>
  CreateNode(const std::string &name) const; ///< Create a node
};

template <typename T> void IXmlNode::Value(const T &value) {
 ...
 */
template <> void IXmlNode::Value(const std::string &value);
