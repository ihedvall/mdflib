...
template <typename T> class LittleBuffer {
public:
...
template <typename T> const uint8_t *LittleBuffer<T>::data() const {
  return buffer_.data();
}

template <typename T> uint8_t *LittleBuffer<T>::data() {
  return buffer_.data();
}
template <typename T> size_t LittleBuffer<T>::size() const {
  return buffer_.size();
}

template <typename T> T LittleBuffer<T>::value() const {
