...
std::vector<uint8_t> MdfHelper::NsToCanOpenTimeArray(uint64_t ns_since_1970)  {
  ...
  const LittleBuffer milli_sec(static_cast<uint32_t>(ms_since_1984 % (24 * 3'600'000)));
...
std::string MdfHelper::FormatDouble(double value, uint8_t decimals, bool fixed, const std::string& unit) {
...
uint64_t MdfHelper::NowNs() {
  ...
  const auto ns_midnight = std::chrono::duration_cast<std::chrono::nanoseconds>(now.time_since_epoch()) % 1000'000'000;
